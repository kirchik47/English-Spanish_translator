...
class CHRF(keras.metrics.Metric):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        indices = keras.ops.concatenate(
            [
                keras.ops.arange(i, i + y_true.shape[1] - self.n_grams + 1)[:, tf.newaxis]
                for i in range(self.n_grams)
            ],
            axis=-1,
        )
        references = keras.ops.concatenate(
            [keras.ops.take(y_true, indices[i], axis=-1) for i in range(indices.shape[0])], axis=0
        )
        indices = keras.ops.concatenate(
            [
                keras.ops.arange(i, i + y_pred.shape[1] - self.n_grams + 1)[:, tf.newaxis]
                for i in range(self.n_grams)
            ],
            axis=-1,
        )
        predictions = keras.ops.concatenate(
            [keras.ops.take(y_pred, indices[i], axis=-1) for i in range(indices.shape[0])], axis=0
        )
        ...
        num_common_ngrams = keras.ops.cast(
            keras.ops.sum(keras.ops.equal(predictions, references)), tf.float64
        )
        # common_ngrams = tf.sets.intersection([references], [predictions])
        # num_common_ngrams = tf.shape(common_ngrams.values)[0]
        precision = keras.ops.cond(
            keras.ops.shape(predictions)[0] > 0,
            lambda: num_common_ngrams
            / keras.ops.cast(keras.ops.shape(predictions)[0], tf.float64),
            lambda: keras.ops.cast(0.0, tf.float64),
        )
        recall = keras.ops.cond(
            keras.ops.shape(references)[0] > 0,
            lambda: num_common_ngrams / keras.ops.cast(keras.ops.shape(references)[0], tf.float64),
            lambda: keras.ops.cast(0.0, tf.float64),
        )
        f1_score = keras.ops.cond(
            precision + recall > 0,
...
def build_model():
    ...
    encoder_embedding_layer = keras.layers.Embedding(
        vocab_size, output_dim=embed_size, mask_zero=True
    )
...
def train():
    ...
    print(model.summary())
    model.compile(
        optimizer="rmsprop", loss="sparse_categorical_crossentropy", metrics=["accuracy", CHRF(3)]
    )
